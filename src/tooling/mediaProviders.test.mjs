...
function fixture(t, id) {
  ...
  writeFileSync(path.join(root, 'config/cctv_sources.austin.json'), JSON.stringify([
    { id, name: '<Camera & test>', lat: 30.27, lon: -97.74, feedType: 'video' },
  ]));
  return root;
}

function isolate(t) {
  for (const name of ['CCTV_SOURCES_FILE', 'CCTV_SOURCES_JSON', 'CCTV_FORCE_AUSTIN', 'GOOGLE_MAPS_SERVER_API_KEY', 'GOOGLE_MAPS_API_KEY']) {
  ...
  t.mock.method(globalThis, 'fetch', () => { throw Error('fixture must not fetch'); });
}
...
test('CCTV instances resolve their own application source root and isolate catalogs and health', async (t) => {
  ...
  assert.deepEqual(JSON.parse(a.body).sources.map(s => s.id), ['first']);
  assert.deepEqual(JSON.parse(b.body).sources.map(s => s.id), ['second']);
