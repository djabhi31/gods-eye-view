import { radioTunerSlot, radioTunerCommitSlot, radioTunerPointerPosition, buildRadioTunerTicks } from './radioTunerModel.js';
...
export function bindRadioControls() {
    ...
    const tunerPreview = ({ coordinate = this._radioTunerCoordinate, syncStatic = true, rotate = true } = {}) => {
      ...
      if (syncStatic) this.radio.previewTuningStation(station?.id || null, { rotate });
      return station;
    };
    ...
    const beginTuner = () => {
      ...
      refreshTunerBand();
      if (!this._radioTunerStations.length || !this.radio.beginTuning()) return false;
    ...
    const toggleRadio = async (trigger) => {
      ...
      try {
        const toggled = await this.actions.runUserAction(
          (notificationToken) => this.actions.setEnabled(enabling, {
            origin: 'user',
            notificationToken,
          }),
    ...
    this.listen(this._radioEnableBtn, 'click', () => void toggleRadio(this._radioEnableBtn));
    this.listen(this._contextRadioMiniEnableBtn, 'click', () => void toggleRadio(this._contextRadioMiniEnableBtn));
    this.listen(this._cockpitRadioEnableBtn, 'click', () => void toggleRadio(this._cockpitRadioEnableBtn));
    ...
    this.listen(this._contextRadioDetailsBtn, 'click', () => {
      if (!this.actions.isCockpitActive()) this.actions.setPanelCollapsed('global-context-panel', false, { explicit: true });
      this.actions.setPanelCollapsed('radio-panel', false, { explicit: true });
      setRadioDisclosure(false);
    ...
    this.listen(window, 'gev:cockpit-mode-changed', (event) => {
      if (event?.detail?.active) return;
      setCockpitDisclosure('display', false);
      setCockpitDisclosure('radio', false);
    }, tunerListenerOptions);
    this.listen(window, 'gev:cockpit-signal-expanded', () => {
      setCockpitDisclosure('display', false);
    }, tunerListenerOptions);
    this.listen(window, 'gev:cockpit-context-expanded', () => {
    ...
    this.listen(this._radioPlayBtn, 'click', () => void this.radio.togglePlayback({ origin: 'user' }));
    this.listen(this._radioStopBtn, 'click', () => this.radio.stopPlayback({ origin: 'user' }));
    ...
    this.listen(this._contextRadioMiniPlayBtn, 'click', () => void this.radio.togglePlayback({ origin: 'user' }));
    ...
    this.listen(this._cockpitRadioPrevBtn, 'click', () => cycleRadio(-1, { rotate: false }));
    this.listen(this._cockpitRadioNextBtn, 'click', () => cycleRadio(1, { rotate: false }));
    this.listen(this._cockpitRadioPlayBtn, 'click', () => void this.radio.togglePlayback({ origin: 'user' }));
    ...
    this.listen(this._radioTunerSlider, 'pointerdown', (event) => {
    ...
    this.listen(this._radioTunerSlider, 'pointermove', (event) => {
      if (!this._radioTunerDragging || this._radioTunerPointerId !== event.pointerId) return;
      updateTunerFromPointer(event);
      event.preventDefault();
    }, tunerListenerOptions);
    this.listen(this._radioTunerSlider, 'input', () => {
    ...
    this.listen(this._radioTunerSlider, 'change', () => {
      if (this._radioTunerPointerId === null && !this._radioTunerKeyboardKey) finishTuner(true);
    }, tunerListenerOptions);
    this.listen(this._radioTunerSlider, 'pointerup', (event) => {
    ...
    this.listen(this._radioTunerSlider, 'keyup', (event) => {
      if (!this._radioTunerKeyboardKey || event.key !== this._radioTunerKeyboardKey) return;
      event.preventDefault();
      finishTuner(true);
    }, tunerListenerOptions);
    this.listen(this._radioTunerSlider, 'blur', () => finishTuner(true), tunerListenerOptions);
    const releaseNavigationBand = () => {
      this._radioTunerBandPinnedForNavigation = false;
    };
    this.listen(this.canvas, 'pointerdown', releaseNavigationBand, tunerListenerOptions);
    this.listen(this.canvas, 'wheel', releaseNavigationBand, tunerListenerOptions);
    ...
    this._radioSelectedHandler = () => this.actions.setPanelCollapsed('radio-panel', false);
