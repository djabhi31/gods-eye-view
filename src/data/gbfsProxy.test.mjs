...
function endlessResponse(chunk, onCancel) {
  ...
  return { response: new Response(stream, { status: 200 }), pulls: () => pulls };
}
...
test('GBFS upstream fetch passes a direct 200 through unchanged', async () => {
  ...
  assert.equal(observedOptions.redirect, 'manual', 'fetch must not follow redirects on its own');
...
test('GBFS upstream fetch forwards a non-redirect error status for the middleware to relay', async () => {
  ...
  assert.equal(result.contentType, 'application/json', 'a missing upstream content type defaults to JSON');
      ...
      fetchGbfsUpstream(STATION_URL, {
        fetchImpl: async () => {
          ...
          return new Response('moved', {
            status,
            headers: { Location: 'https://attacker.example/station_status.json' },
          });
        },
      }),
      (error) => error?.code === 'GBFS_REDIRECT' && error.redirectHost === 'attacker.example',
      `status ${status} must be refused`,
    );
    assert.equal(calls, 1, `status ${status}: the redirect target must not be requested`);
  }
});
...
test('GBFS upstream fetch names the host of a relative redirect', async () => {
  await assert.rejects(
    fetchGbfsUpstream(STATION_URL, {
      fetchImpl: async () => new Response(null, { status: 302, headers: { Location: '/v2/station_status.json' } }),
    }),
    (error) => error?.code === 'GBFS_REDIRECT' && error.redirectHost === 'gbfs.lyft.com',
  );
});
...
test('GBFS upstream fetch rejects an oversized Content-Length before reading the body', async () => {
  ...
  assert.equal(pulls(), 0, 'the body must not be read when the declared length already exceeds the cap');
});

test('GBFS upstream fetch cancels a length-less body the moment it passes the cap', async () => {
  let cancelled = false;
  const chunk = new Uint8Array(1024);
  const { response, pulls } = endlessResponse(chunk, () => { cancelled = true; });
  await assert.rejects(
    fetchGbfsUpstream(STATION_URL, { fetchImpl: async () => response, maxBytes: 4 * chunk.byteLength }),
    (error) => error?.code === 'RESPONSE_TOO_LARGE',
  );
  assert.equal(cancelled, true, 'the upstream stream must be cancelled, not drained');
  assert.ok(pulls() <= 8, `only a few chunks may be read past the cap, saw ${pulls()}`);
});
